...
class ASEBuilder(object):
    def build(self, context, options: ASEBuilderOptions, object : bpy.types.Object, ase: ASE):
        ...
        if not is_collision_name(object.name) and main_geometry_object is not None:
            geometry_object = main_geometry_object
        else:
            geometry_object = ASEGeometryObject()
            geometry_object.name = object.name
        ...
        if geometry_object.is_collision:
            ...
            for edge in bm.edges:
                if not edge.is_manifold:
                    del bm
                    raise ASEBuilderError(f'Collision mesh \'{object.name}\' is not manifold')
                if not edge.is_convex:
                    del bm
                    raise ASEBuilderError(f'Collision mesh \'{object.name}\' is not convex')

        if not geometry_object.is_collision and len(object.data.materials) == 0:
            raise ASEBuilderError(f'Mesh \'{object.name}\' must have at least one material')
        ...
        material_indices = []
        if not geometry_object.is_collision:
            for mesh_material_index, material in enumerate(object.data.materials):
                if material is None:
                    raise ASEBuilderError(f'Material slot {mesh_material_index + 1} for mesh \'{object.name}\' cannot be empty')
                try:
